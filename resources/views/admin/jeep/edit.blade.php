...
@section('title', 'Edit Jeep - Jeep Dieng')
@section('header_title', 'Edit Armada Jeep')
@section('header_subtitle', 'Perbarui informasi kendaraan')

@section('content')
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 max-w-2xl mx-auto">
    <form action="{{ route('admin.jeep.update', $jeep->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="komunitas_id" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition">
                ...
                @foreach($komunitas as $kom)
                    <option value="{{ $kom->id }}" {{ $jeep->komunitas_id == $kom->id ? 'selected' : '' }}>{{ $kom->nama_komunitas }}</option>
                @endforeach
            </select>
        </div>
        @endif
        ...
        <div>
            ...
            <input type="text" name="nama_jeep" value="{{ $jeep->nama_jeep }}" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition">
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Nomor Polisi <span class="text-red-500">*</span></label>
                <input type="text" name="nomor_polisi" value="{{ $jeep->nomor_polisi }}" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition uppercase">
        ...
        <div>
            ...
            <select name="status" required class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition">
                <option value="Tersedia" {{ $jeep->status == 'Tersedia' ? 'selected' : '' }}>Tersedia</option>
                <option value="Disewa" {{ $jeep->status == 'Disewa' ? 'selected' : '' }}>Disewa</option>
                <option value="Perbaikan" {{ $jeep->status == 'Perbaikan' ? 'selected' : '' }}>Perbaikan (Maintanance)</option>
            </select>
        </div>
