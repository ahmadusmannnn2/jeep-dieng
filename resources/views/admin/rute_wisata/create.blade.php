...
@section('header_title', 'Tambah Rute Wisata')
@section('header_subtitle', 'Masukkan detail destinasi perjalanan baru')

@section('content')
<div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-8 max-w-2xl mx-auto">
    <form action="{{ route('admin.rute-wisata.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="nama_rute" required placeholder="Contoh: Basecamp - Batu Pandang - Kawah Sikidang" class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Deskripsi Rute</label>
            <textarea name="deskripsi" rows="4" placeholder="Jelaskan objek wisata yang dilalui..." class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition"></textarea>
        </div>
